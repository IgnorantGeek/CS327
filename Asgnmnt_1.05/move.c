...
//add check for PC, if char is PC, read move from getch()
//move from getch() can either be passed in or not.
void move_character(dungeon_t *d, character_t *c, pair_t next)
{
  if (c == &d->pc)
  {
    //check that next move is valid
  }
  if (charpair(next) &&
...
void do_moves(dungeon_t *d)
{
  ...
  //this processes a pc move
  //shouldnt need to modify this one
  if (pc_is_alive(d) && e->c == &d->pc) 
  {
    ...
    e->c = NULL;
    event_delete(e);
    pc_next_pos(d, next);//what does this do?
    next[dim_x] += c->position[dim_x];
    next[dim_y] += c->position[dim_y];
    move_character(d, c, next); //this is the code that actually moves the character
    dijkstra(d);
    dijkstra_tunnel(d);
  }
}

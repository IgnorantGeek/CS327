...
int gen_dungeon(dungeon_t *d)
{
  ...
  int place;
  int stairup = rand() % 2;
  int stairdown = rand() % 2;
  if (stairup)
  {
    place = rand_range(0, d->num_rooms);
    room_t room = d->rooms[place];
    mapxy(rand_range(room.position[dim_x],room.size[dim_x]-1),rand_range(room.position[dim_y],
          room.size[dim_y]-1)) = '<';
  }
  if (stairdown)
  {
...
void monsters_list(dungeon_t *d)
{
  //pair_t p;
  WINDOW *mon = newwin(10, 10, 0, 0);
  keypad(mon, TRUE);
  short sh = 0;
  character_t monsters[d->num_monsters];
  int i, x, y, topscr, botscr, monscr;
  monscr = 10;
  topscr = 0; //stores the postion of the top and bottom of monster screen
  botscr = monscr - 1;
  i = 0;
  for (x = 0; x < DUNGEON_X; x++)
  {
    for (y = 0; y < DUNGEON_Y; y++)
    {
      if (charxy(x,y) && x != d->pc.position[dim_x] && y != d->pc.position[dim_y])
      {
        monsters[i] = *charxy(x,y);
        i++;
      }
    }
  }
  while (sh != 27)
  {
    ...
    {
      case KEY_UP:
        if (d->num_monsters > monscr && topscr != 0)
        {
          topscr--;
          botscr--;
        }
      ...
      case KEY_DOWN:
        if (d->num_monsters > monscr && botscr != d->num_monsters - 1)
        {
          topscr++;
          botscr++;
        }
    ...
    int listcount, xdist, ydist;
    listcount = 0;
    for (i = topscr; i <= botscr; i++) 
    {
      xdist = ydist = 0;
      char *xcard = malloc(sizeof(char) *5);
      char *ycard = malloc(sizeof(char) *5);
      move(listcount, 0);
      xdist = d->pc.position[dim_x] - monsters[i].position[dim_x];
      ydist = d->pc.position[dim_y] - monsters[i].position[dim_y];
      if (xdist > 0)
      {
        xcard = "West";
      }
      ...
      printw("%c, %d %s and %d %s", monsters[i].symbol, abs(ydist), ycard, abs(xdist), xcard);
      listcount++;
    }
    sh = getch();
  }

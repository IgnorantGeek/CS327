...
static int xMove[8] = {1, 1, 0,-1,-1,-1, 0, 1};
static int yMove[8] = {0, 1, 1, 1, 0,-1,-1,-1};
...
static event_t new_event(char_t character)
{
  event_t new;
  new.time = time(NULL); //this is probably wrong, need to make sure I am calling correct func
  new.turn = 0;
  new.character = &character;//make sure this is right
  return new;
}
...
{
  d->num_mon = num_mon;
  d->monsters = malloc(sizeof(*d->rooms) * num_mon);
  //generate num_mon number of monsters
  //need to have code to randomly place monsters in rooms, not the room the PC is in, can be done here.
  //code to generate monster type, 50% chance per monster characteristic
  int i;
  for (i = 0; i < num_mon; i++)
  {
    ...
    int erratic = rand() % 2;
    char_t newmon;
    newmon.speed = rand_range(5, 20);
    pair_t place; //position to place monster in
    int x = rand_range(0, d->num_rooms - 1);
    while (x == get_room(d, d->pc.position))
    {
      x = rand_range(0, d->num_rooms - 1);
    } //should loop through only if the room assignment is the same room as pc
    int exit = 1;
    //need to add check for monsters getting placed on top of other monsters
    while (exit == 1)//possible bug here: infinite loop if generating max monsters
    {
      //generate random point, if point is filled, generate new random point
      place[dim_y] = rand_range(d->rooms[x].position[dim_y], d->rooms[x].position[dim_y] + d->rooms[x].                                size[dim_y] - 1);
      place[dim_x] = rand_range(d->rooms[x].position[dim_x], d->rooms[x].position[dim_x] + d->rooms[x].                                size[dim_x] - 1);
      if (mappair(place) == ter_floor_room)
      {
        exit = 0;
      }
      int z;
      for (z = 0; z < d->num_mon; z++)
      {
        if (place[dim_y] == d->monsters[i].position[dim_y] &&
            place[dim_x] == d->monsters[i].position[dim_x])
        {
          exit = 1;
        }
      }
    }
    memcpy(&newmon.position, &place, sizeof(pair_t));
    ...
    if (intelligence == 1)
    {
      memcpy(newmon.Intel, d->pc.position, sizeof(pair_t));
    }
    new_event(newmon);
    d->monsters[i] = newmon;
  }
...
{
  //return the room of the pair
  //returns -1 if pair is not in a room
  int i;
  for (i = 0; i < d->num_rooms; i++)
  {
    if (pos[dim_y] >= d->rooms[i].position[dim_y] && pos[dim_x] >= d->rooms[i].position[dim_x] &&
...
//add casts so you are comparing ints
static char is_character(dungeon_t *d, pair_t p)
{
  //returns 0 if point is char, 1 otherwise
  if (p[dim_x] == d->pc.position[dim_x] && p[dim_y] == d->pc.position[dim_y]) return '@';
  int i;
  for (i = 0; i < d->num_mon; i++)
  {
    if (p[dim_x] == d->monsters[i].position[dim_x] && p[dim_y] == d->monsters[i].position[dim_y])
      return d->monsters[i].type;
  }
  return 'z';
}

//what is stored at map(y)(x) when a character is present?
//map might only be storing terrain types, if so I can still print this way
void render_dungeon(dungeon_t *d)
{
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++)
  {
    ...
    {
      if (is_character(d, p) != 'z')
      {
        putchar(is_character(d, p));
      }
      else
      {
        switch (mappair(p))
...
{
  if (pos1[dim_x] == pos2[dim_x] && pos1[dim_y] == pos2[dim_y]) return 0;
  else return 1;
}
...
//TODO
//This will process how to move the character and proceed with the move.
void process_move(dungeon_t *d, char_t *character)
{
  //split into two types, tunneling and non
  //figure out how to handle character collisions, if there is about to be a colision, how do we
  //check that? Might need variable for nextPos as well as currPos
  ...
  switch (character->type)
  {
    case '@':
      break;
    case '0':
      if (get_room(d, character->position) == get_room(d, d->pc.position))
      {
        int i, nx, ny, lowest;
        lowest = INT_MAX;
        for (i = 0; i < 8; i++)
        {
          if (d->pc_distance[character->position[dim_y] + yMove[i]][character->position[dim_x] +xMove[i]]
              < lowest)
          {
            lowest = d->pc_distance[character->position[dim_y] + yMove[i]]
              [character->position[dim_x] +xMove[i]];
            nx = xMove[i];
            ny = yMove[i];
          }
        }
        character->position[dim_y] += ny;
        character->position[dim_x] += nx;
      }
      else
      {
        int16_t nx = rand_range(-1,1);
        int16_t ny = rand_range(-1,1);
        if (hardnessxy(character->position[dim_x] + nx, character->position[dim_y] + ny) != 0)
        {
        }
        else
        {
      ...
      break; //no attributes (random move, or towards if same room)
    case '1':
      ; //erratic only (essentially random)
    case '2':
      ; //Tunneling only
    case '3':
      ; //tunneling and erratic
    case '4':
      ; //telepathy only
    case '5':
      ; //Telepathy and erratic
    case '6':
      ; //telepathy and tunneling
    case '7':
      ; //telepathy, tunneling, erratic
    case '8':
      ; //intelligent only
    case '9':
      ; //intelligent and erratic
    case 'a':
      ; //intelligent and tunneling
    case 'b':
      ; //intelligent, tunneling, erratic
    case 'c':
      ; //Intelligent and telepathy
    case 'd':
      ; //intelligent, telepathy, and erratic
    case 'e':
      ; //intelligent, telepathy, and tunneling
    case 'f':
      if (character->type == 'f')//arbitrary if, satisfy case error
      {
        int nx, ny, i, lowest;
        lowest = INT_MAX;
        for (i = 0; i < 8; i++)
        {
          if (d->pc_tunnel[character->position[dim_y] + yMove[i]][character->position[dim_x] + xMove[i]] < lowest)
          {
            lowest = d->pc_tunnel[character->position[dim_y] + yMove[i]][character->position[dim_x] + xMove[i]];
            nx = character->position[dim_x] + xMove[i];
            ny = character->position[dim_y] + yMove[i];
          }

        }
        if (mapxy(nx,ny) == ter_wall)
        {
          mapxy(nx,ny) = ter_floor_hall;
        }
        character->position[dim_x] = nx;
        character->position[dim_y] = ny;
      }
      break;//all attributes
  }
}
